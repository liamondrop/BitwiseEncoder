(function (global) {
  ...
  function _checkRange(range) {
    for (var args = Array.prototype.slice.apply(arguments), i = 1, l = args.length, a; i < l; i++)
      if (a = args[i], a < range[0] || a > range[1])
        throw new RangeError('value (' + a + ') out of range [' + range.join(' .. ') + ']')
  }
